...
// Build Wokwi dengan:
// arduino-cli compile --fqbn esp32:esp32:esp32doit-devkit-v1 --output-dir .build --build-property build.extra_flags="-DWOKWI_SIMULATION=1" .
...
void loop() {
  ...
  if (automationEnabled && beratPakan >= 0 && beratPakan <= BATAS_MINIMAL_PAKAN) {
  ...
  // LOGIKA OTOMATIS AIR
  if (automationEnabled && beratMinum >= 0 && beratMinum <= BATAS_MINIMAL_AIR) {
